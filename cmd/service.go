...
func newServiceCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			r.With(handler.RequireAuth).Put("/roles/{id}", roleHandler.Update)
			r.With(handler.RequireAuth).Delete("/roles/{id}", roleHandler.Delete)
			r.Get("/roles/{roleID}/permissions", permissionHandler.ListByRole)

			r.With(handler.RequireAuth).Post("/permissions", permissionHandler.Create)
			r.Get("/permissions/{id}", permissionHandler.GetByID)
			r.With(handler.RequireAuth).Put("/permissions/{id}", permissionHandler.Update)
			r.With(handler.RequireAuth).Delete("/permissions/{id}", permissionHandler.Delete)
			...
			r.With(handler.RequireAuth).Post("/composites", compositeHandler.Create)
			r.Get("/composites/slug/{slug}", compositeHandler.GetBySlug)
			r.Get("/composites/{id}", compositeHandler.GetByID)
			r.With(handler.RequireAuth).Put("/composites/{id}", compositeHandler.Update)
			r.With(handler.RequireAuth).Delete("/composites/{id}", compositeHandler.Delete)

			r.Get("/composites/{compositeID}/fields", fieldHandler.ListByComposite)
			r.With(handler.RequireAuth).Post("/fields", fieldHandler.Create)
			r.Get("/fields/{id}", fieldHandler.GetByID)
			r.With(handler.RequireAuth).Put("/fields/{id}", fieldHandler.Update)
			r.With(handler.RequireAuth).Delete("/fields/{id}", fieldHandler.Delete)
			...
			r.With(handler.RequireAuth).Put("/entities/{id}", entityHandler.Update)
			r.With(handler.RequireAuth).Delete("/entities/{id}", entityHandler.Delete)
			...
			r.With(handler.RequireAuth).Delete("/field-values/{id}", fieldValueHandler.Delete)
