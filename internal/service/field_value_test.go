...
func TestFieldValueService_Set(t *testing.T) {
	...
	t.Run("delegates to repo upsert", func(t *testing.T) {
		var upsertedValue string
		svc := service.NewFieldValueService(&mockFieldValueRepo{
			upsert: func(_ context.Context, fv *models.FieldValue) (*models.FieldValue, error) {
				upsertedValue = fv.Value
				return fv, nil
			},
		}, &mockFieldService{})
		...
		if upsertedValue != "new value" {
			t.Errorf("expected upserted value %q, got %q", "new value", upsertedValue)
		}
	})
}
...
func TestFieldValueService_Validate(t *testing.T) {
	...
	noopRepo := &mockFieldValueRepo{
		upsert: func(_ context.Context, fv *models.FieldValue) (*models.FieldValue, error) { return fv, nil },
	}
